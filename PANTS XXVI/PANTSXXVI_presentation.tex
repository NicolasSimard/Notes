...
\frame{
	...
	\begin{theorem}
		The space $S_k(\Gamma_0(N),\chi)^{\text{new}}$ has an \emph{orthogonal} basis of so called newforms (after suitable normalization). Those newforms are eigenvalues for all Hecke operators.
	\end{theorem}
}
...
\frame{
	...
	\begin{theorem}
		...
		\[\langle\tpsi,\tpsi\rangle = (|D|/4)^\ell\frac{4h_K}{w_K^2}\sum_{[\ida]\in\ClK}\psi^2(\ida)\partial^{2\ell-1}G_2(\ida).\]
	\end{theorem}
	Here,
...
\frame{
	\frametitle{Compute $\partial^nG_2$}
...
\frame{
	\frametitle{Evaluate Hecke characters}
...
\frame{
	\frametitle{Find ideals $\idc$ such that $\ida\idb^{-1}\idc^2=\lambda_\idc\O_K$}
...
\frame{
	\frametitle{$K=\Q(\sqrt{-23})$ (class number $3$, one genus)}
...
\frame{
	\frametitle{Example of computation: $K=\Q(\sqrt{-23})$, $N(\psi_0,3)$}
	\begin{figure}
...
\frame{
	...
	\begin{enumerate}
		\item<1-> Formulas for the Petersson inner products of theta series in terms of derivatives of Eisenstein series int the case $\ell>0$.
		\item<2-> Formulas for the Petersson inner product of cuspidal weight one theta series.
		\item<3-> An algorithm to compute those quantities.
	\end{enumerate}
}
...
\frame{
	...
	\begin{enumerate}
		\item<1-> Can we say something about the Petersson inner product of non-cuspidal weight one theta series?
		\item<2-> Can we explain what can be ovserved from the computations?
		\item<3-> What are the $p$-adic properties of those quantities as $\ell$ varies? In particular, does the case $\ell>0$ tend to the case $\ell=0$ $p$-adically?
	\end{enumerate}
}
...
\frame{
	...
	Notes available at: \url{https://github.com/NicolasSimard/Notes}
}
