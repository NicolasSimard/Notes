...
\frame{
	\frametitle{Compute $\partial^nE_2$}
...
\frame{
	...
	\begin{tabular}{|c|c|}
	\hline
	$\ell$ & $\det(V_D\langle\theta_{\ida_i}^{(2\ell)},\theta_{\ida_j}^{(2\ell)}\rangle)_{\ida_i,\ida_j\in\ClK}/(\Omega_K^{4\ell})^3$ \\
	\hline
	$1$ & $-2^{10}23$ \\
	\hline
...
\frame{
	...
	\begin{tabular}{|c|c|}
	\hline
	$\ell$ & $\det(V_D\langle\theta_{\psi_i},\theta_{\psi_j}\rangle)_{1\leq i,j\leq 3}/(\Omega_K^{4\ell})^3$ \\
	\hline
	$1$ & $-3^{3}23$ \\
	\hline
...
\frame{
	...
	\begin{enumerate}
		...
		\[\langle\tpsi,\tpsi\rangle=\textcolor{gray}{V_D^{-1}\frac{4h_k}{w_k}\sqrt{|D|}\frac{\Gamma(2\ell+1)}{(4\pi)^{2\ell+1}}}L(\psi^2,2\ell+1).\]
		...
		\[\partial^{2\ell-1}E_2(z)=\textcolor{gray}{(-4\pi)^{1-2\ell}\frac{\Gamma(s+2\ell+1)}{\Gamma(s+2)}}G_{4\ell}(z,1-2\ell).\]
