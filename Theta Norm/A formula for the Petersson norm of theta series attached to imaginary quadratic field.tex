...
so
\[\det M_{\{\O_K,\ida\}}=N(\ida)^{2\ell}\langle\theta_{\O_K},\theta_{\O_K}\rangle^2.\]
...
\subsection{Example with class number 3: $D=-23$}
In this seciton, we consider the case $K=\Q(\sqrt{-23})$, which has cyclic class group of order $3$ generated by one of the primes above $2$, say $\p_2$.
...
For $\ell=1,2$ and $4$, we find that $a(\ell)^3$ is a root of a degree $3$ polynomial and generates the Hilbert class field over $K$. For example, we find that $a(1)$ is a root of the polynomial
...
For $\ell=1,2$ and $4$, $b(\ell)^3$ is a root of a degree $6$ polynomial. For example, $b(1)$ is a root of
...
\textbf{PUT THE COMPUTATIONS.}

It is surprising at first that althought the entries are algebraic of high degree (up to $18$) and belong to different fields, the determinant is rational.
\subsection{$D=-104$}
