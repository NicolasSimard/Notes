...
\[\langle\tpsi,\tpsi\rangle=V_D^{-1}(|D|/4)^\ell\frac{h_K}{w_K^2}\sum_{[\ida]\in\Clk}\psi^2(\ida)del_2^{2\ell-1}E_{2}(\ida)\]
if $\ell>0$ and
...
\begin{equation}\label{eq:filall=0}
	\langle\tpsi,\tpsi\rangle =-V_D^{-1}\frac{4h_K}{w_K^2}\sum_{[\ida]\in\Clk}\psi^2(\ida)\log(\Im(\tau_\ida)^{1/2}|\eta(\tau_\ida)|^2),
\end{equation}
where $V_D$ is defined as
\[V_D=\text{Vol}(\Gamma_0(|D|)\setminus\H)=\text{Vol}(\SL\setminus\H)[\SL:\Gamma_0(|D|)]=\frac{\pi}{3}|D|\prod_{p|D}(1+p^{-1}).\]
...
is the usual holomorphic Eisenstein series for $k\geq 4$ (see \cite[Sec 9.2]{Shi1}), one sees that the previous Corollary relates certain special values of the Hecke $L$-function attached to $\psi$ to the derivatives of the usual Eisenstein series.

Applying the previous Corollary to $\psi^2$ with $m=2\ell+1$, we get
\[L(\psi^2,2\ell+1)=\frac{1}{w_K}\frac{(-4\pi)^{2\ell-1}}{\Gamma(2\ell+1)}\left (\frac{\sqrt{|D|}}{2}\right )^{2\ell-1}\sum_{[\ida]=[\latbasis]}\frac{\psi^2(\ida)}{\omega_2^{4\ell}}\del^{2\ell-1}G_{2}(\omquot,0).\]

Using this value of $L(\psi^2,2\ell+1)$ in formula \ref{NormLpsi} and simplifying finally gives
\[\langle\tpsi,\tpsi\rangle = V_D^{-1}\frac{|D|^\ell}{2^{2\ell}}\frac{h_K}{w_K^2}\frac{-2}{(2\pi)^2}\sum_{[\ida]=[\latbasis]}\frac{\psi^2(\ida)}{\omega_2^{4\ell}}\del^{2\ell-1}G_{2}(\omquot,0),\]
which can be rewritten as
\begin{equation}\label{eq:finallgeq1}
\langle\tpsi,\tpsi\rangle=V_D^{-1}(|D|/4)^\ell\frac{h_K}{w_K^2}\sum_{[\ida]=[\latbasis]}\frac{\psi^2(\ida)}{\omega_2^{4\ell}}\del^{2\ell-1}E_{2}(\omquot).
\end{equation}

Note that this can also be written in homogeneous form as
\[\langle\tpsi,\tpsi\rangle=V_D^{-1}(|D|/4)^\ell\frac{h_K}{w_K^2}\sum_{[\ida]}\psi^2(\ida)\del^{2\ell-1}E_{2}(\ida).\]
