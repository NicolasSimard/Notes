...
Recall that an oriented lattice is a lattice $\ida$ equipped with a $\Z$-basis $\latbasis$, where the order of the basis elements is important. If $\Im(\omquot)>0$, $\ida$ is called positively oriented. If the $\Z$-basis $\latbasis$ is not positively oriented, the basis $[\omega_2,\omega_1]$ is, so that any lattice $\ida$ can be positively oriented. Note that we do not make any holomorphy assumptions on $f$.
