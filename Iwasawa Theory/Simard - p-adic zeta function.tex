...
\[\lambda\mapsto\lambda_\Hfrak=\sum_{x\in\Gfrak/\Hfrak}c_\Hfrak(x)x:\Dist(\Gfrak,A)\rarr\Lambda(\Gfrak/\Hfrak,A).\]
...
In other words, the elements of $\Gfrak$ in $\Lambda(\Gfrak)$ correspond to Dirac distributionsq.
...
At this point, the construction of the $p$-adic zeta function is almost a formality! First, recall that
\[\curlK_n=\Q_p(\mu_{p^{n+1}}\]
and that $\curlU_n$ is the set of units in $\curlK_n$ ($n=\geq 0$). As usual, fix a generator $(\zeta_n)$ of the Tate module $T_p(\mu)$, so that
\[\zeta_n\in\curlU_n\myand \zeta_n^p=\zeta_{n-1}\text{ for }n\geq 1.\]
For integers $a$ and $b$, define
\[c_n(a,b)\frac{\zeta_n^{-a/2}-\zeta_n^{a/2}}{\zeta_n^{-b/2}-\zeta_n^{b/2}}=\zeta_n^{(a-b)/2}\frac{\zeta_n^{-a}-1}{\zeta_n^{-b}-1}.\]
If $a$ and $b$ are not divisible by $p$, the $c_n(a,b)$ is a unit in $\curlF_n$, hence in $\curlK_n$ (this follows from \cite[Proposition 6.2(c)]{Mi}). We claim that the sequence
\[c(a,b)\eqdef(c_n(a,b))\]
...
where $N_{n,n-1}:\curlK_n\rarr\curlK_{n-1}$ is the norm map. This follows from the fact that $\zeta_n$ is a root of
\[X^p-\zeta_{n-1},\]
which is irreducible over $\curlK_{n-1}$ since
\[(X+1)^p-\zeta_{n-1}\]
is Eisenstein at the prime $\pi_{n-1}$. Those elements $c(a,b)$ are the key to defining the $p$-adic zeta function.

Let
\[\lambda(a,b)\eqdef\widetilde{\Res\log}(c(a,b))=\tilde{\curlL}(c(a,b)).\]
Then
\[\int_{\curlG}\chi(g)^k\d\lambda(a,b)=(1-p^{k-1})\delta_k(c(a,b)).\]
The computation of $\delta_k(c(a,b))$ is done in \cite[Proposition 2.6.3]{CS}:
\[
...
is a pseudo-measure on $\curlG$ which is independent of the choice of $a$ and $b$. More importantly,
\[
\int_{\curlG}\chi(g)^k\d\tilde{\zeta}_p=\begin{cases}
	0	&\text{if }k=1,3,5,\dots\\
	\zeta(1-k)&\text{if }k=2,4,6,\dots\\
\end{cases}
\]
