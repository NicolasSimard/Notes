...
\frame{
	...
	It is a fascinating fact that many $L$-functions contain arithmetic information in their value at $s=1$:
	\begin{enumerate}
...
\frame{
	...
	\begin{enumerate}[<+->]
		...
		\[\res_{s=2\ell+1}L(\Sym^2 \theta_\psi,s)=\textcolor{gray}{\prod_{p|D}(1-p^{-1})}L(\psi^2,2\ell+1)\]
...
\frame{
	...
	\[F:\cW\rightarrow\C_p\]
	with the property that for all $\ell >  0$,
	\[\frac{F(\ell)}{\Omega_p(\fc)^{4\ell}} = (\Frob_\fp^{-1}-p^{2\ell-1})(\Frob_\fp^{-1}-p^{2\ell})\left (\frac{\langle\theta_{\fa,\ell},\theta_{\fb,\ell}\rangle}{((2\pi i)^{-1}\Omega_\C(\fc))^{4\ell}}\right ),\]
...
\frame{
	...
	\begin{tabular}{|>{\centering\arraybackslash\hspace{0pt}}p{0.1\linewidth}|>{\centering\arraybackslash\hspace{0pt}}p{0.4\linewidth}|>{\centering\arraybackslash\hspace{0pt}}p{0.4\linewidth}|}
    	...
    	\newline$\ell>0$
    	&\cellcolor{Green}$C_1\sum\limits_{\cA\in\ClK}\psi^2(\cA)\delta^{2\ell-1} E_2(\cA)$
    	&\cellcolor{Green}\newline$4(|D|/4)^\ell\delta^{2\ell-1}E_2(\fc)$
    	\\
    	\hline
    	\multirow{2}{*}{$\ell=0$}&\cellcolor{Green}$C_3\sum\limits_{\cA\in\ClK}\psi^2(\cA)\log \Phi(\cA)$&\cellcolor{Red}\\
    	\cline{2-2}
...
\frame{
	\frametitle{Relation to the other invariants}
	After running some experiments, one notices that numerically,
	\[\prod_{i=1}^{h_K}(12\sqrt{|D|}E_2(\fa_i)N(\fa_i)) = n(K)\Omega_K^{2h_K}\]
	for some \emph{integer} $n(K)$.
	
	\only<2->{It follows that $B(K,\ell)$ and $C(K, \ell)$ only differ by $n(K)$, up to an explicit quantity.}
}

\section{Conclusion}
\frame{
	\frametitle{Take home}
	The Petersson inner product of theta series
	\begin{enumerate}[<+->]
		\item Can be efficiently computed,
		\item Can be $p$-adically interpolated,
		\item Contains interesting information in weight $1$ and greater.
	\end{enumerate}
}
...
\frame{
	...
	Presentation available at: \url{https://github.com/NicolasSimard/Notes}
